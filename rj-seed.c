...
void Perturb(Params *P, int nsamp, int fixmodel)
{
  ...
  if(!fixmodel && RND < 0.5 && newgermline != P->germline)
    {
      // these various i->j blocks implement the transition rules for shifting parameters between models
      theta[0] = P->n[0]; theta[1] = P->n[1]; theta[2] = P->n[2];
      if(P->germline == 0 && newgermline == 1)
	{
	  P->n[0] = theta[0];
	  P->n[1] = theta[1];
	  P->n[2] = theta[0]+theta[1]+theta[2];
	}
      if(P->germline == 0 && newgermline == 2)
	{
	  P->n[0] = theta[0];
	  P->n[1] = theta[0]+theta[1];
	  P->n[2] = theta[0]+theta[1]+theta[2];
	}
      if(P->germline == 1 && newgermline == 0)
	{
	  P->n[0] = theta[0];
	  P->n[1] = theta[1];
	  P->n[2] = theta[2]-theta[1]-theta[0];
	}
      if(P->germline == 1 && newgermline == 2)
	{
	  P->n[0] = theta[0];
	  P->n[1] = theta[0]+theta[1];
	  P->n[2] = theta[2];
	}
      if(P->germline == 2 && newgermline == 0)
	{
	  P->n[0] = theta[0];
	  P->n[1] = theta[1]-theta[0];
	  P->n[2] = theta[2]-theta[1];
	}
      if(P->germline == 2 && newgermline == 1)
	{
	  P->n[0] = theta[0];
	  P->n[1] = theta[1]-theta[0];
	  P->n[2] = theta[2];
	}
      // update the germline label
...
int main(int argc, char *argv[])
{
  ...
  sprintf(outfile, "outtrj-%s.csv", paramstr);
  fp = fopen(outfile, "w");
  for(i=0; i<nfamily; i++)
    fprintf(fp, "h0.%i,", i);
  for(i=0; i<NN; i++)
